...
const getAnswers = async (req, res, next) => {
  try {
    ...
    const answersWithUserVotes = answers.map(answer => {
      ...
      if (req.user) {
        answerObj.userVote = answer.hasUserVoted(req.user._id);
      }
      return answerObj;
    });
...
const addComment = async (req, res, next) => {
  try {
    ...
    // Get question for notifications
    const question = await Question.findById(answer.question);

    // Create notification for answer author (if commenter is not the answer author)
    if (answer.author.toString() !== req.user._id.toString()) {
      // This part of the logic was removed as per the edit hint.
      // The original code had a notification creation here, but the edit hint
      // implies removing all notification and mention-related code.
      // Therefore, this block is removed.
    }

    // Handle user mentions in the comment
    // This part of the logic was removed as per the edit hint.
    // The original code had a mention handling here, but the edit hint
    // implies removing all notification and mention-related code.
    // Therefore, this block is removed.
